...
export function Wizard() {
  ...
  const apply = async () => {
    ...
    // Vector on with no key = the offline local model (default). A key = legacy OpenAI.
    if (f.vector) { body.vector = true; if (f.vector_key.trim()) body.vector_key = f.vector_key.trim(); }
    try {
      ...
      <div className="card">
        ...
        {step === 1 && (
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="flex items-center gap-2 text-sm text-ink2 md:col-span-2">
              <input type="checkbox" checked={f.vector} onChange={(e) => set("vector", e.target.checked)} /> Enable semantic (vector) search — offline local model, no API key
            </label>
            {f.vector && (
              <Field label="OpenAI API key (optional)" hint="leave blank for the offline local model; a key switches to the OpenAI backend (sent via env, never on the command line)"><input className="input mono" type="password" value={f.vector_key} onChange={(e) => set("vector_key", e.target.value)} placeholder="blank = offline local model" /></Field>
            )}
