...
cmd_link() {
  ...
  case "${1:-}" in
    ""|--list|list)
      local found=""
      for l in "$ROOT"/raw/*; do
        [ -L "$l" ] || continue
        printf "  %s%s%s -> %s\n" "$C_B" "$(basename "$l")" "$C_0" "$(cd "$l" 2>/dev/null && pwd -P || readlink "$l")"
        found=1
      done
      [ -n "$found" ] || info "no linked sources yet — add one with: ./docmcp.sh link <dir>"
      return ;;
    --remove|--unlink|rm)
      local name="${2:-}"; [ -n "$name" ] || die "usage: ./docmcp.sh link --remove <name>"
      local lp="$ROOT/raw/$name"
      [ -L "$lp" ] || die "no such linked source: '$name' (see: ./docmcp.sh link --list)"
      rm -f "$lp"; info "unlinked '$name' — re-run ./docmcp.sh ingest to drop its docs from the store"
      return ;;
  esac
  local src="$1" name="${2:-}"
  case "$src" in '~'|'~/'*) src="$HOME${src#\~}" ;; esac          # shellcheck disable=SC2088
  [ -d "$src" ] || die "not a directory: $src (link points at a folder of docs)"
  src="$(cd "$src" && pwd -P)" || die "cannot resolve directory: $1"
  # Refuse a target that is the repo root or under raw/: that path is ALREADY an ingest
  # source (mounted at /srv/raw), so linking it would walk the same files twice (/srv/raw
  # AND /srv/linked) and emit duplicate curated docs. ($src is resolved, so this is symlink-free.)
  case "$src/" in
    "$ROOT/raw/"*|"$ROOT/") die "refusing to link '$src': repo root or under raw/ (already an ingest source — would ingest twice)" ;;
  esac
  [ -n "$name" ] || name="$(basename "$src")"
  # No '/' (it would nest under raw/) and no ':' (the name becomes a Docker '-v host:/srv/linked/<name>:ro'
  # mount target; a colon there corrupts the spec — compose truncates at it and links can collide).
  case "$name" in */*|*:*|''|.|..) die "invalid link name: '$name' (no '/' or ':'; it becomes raw/<name> and a mount target)" ;; esac
  local lp="$ROOT/raw/$name"
  [ -e "$lp" ] && [ ! -L "$lp" ] && die "raw/$name already exists and is not a symlink — pick another name"
  ln -sfn "$src" "$lp"
  info "linked ${C_B}raw/$name${C_0} -> $src"
  info "Now run: ./docmcp.sh ingest   (no copy; edits in the folder are picked up on each ingest)"
}
...
cmd_ingest() {
  ...
  # Linked sources (./docmcp.sh link): external folders symlinked into raw/. Bind-mount each
  # link's TARGET into the container at /srv/linked/<name> and add /srv/linked to SOURCE_DIRS,
  # so ingest reads them in place (no copy). The raw/ symlink itself is skipped by the walker;
  # the real content arrives via these mounts, and curated docs land under /<name>/…
  local _vflags=() _linked=""
  if [ -d "$ROOT/raw" ]; then
    for _l in "$ROOT"/raw/*; do
      [ -L "$_l" ] || continue
      local _tgt; _tgt="$(cd "$_l" 2>/dev/null && pwd -P)" \
        || { warn "skipping broken linked source: raw/$(basename "$_l") -> $(readlink "$_l")"; continue; }
      _vflags+=(-v "$_tgt:/srv/linked/$(basename "$_l"):ro")
      info "linked source: raw/$(basename "$_l") -> $_tgt"
      _linked=1
    done
  fi
  [ -n "$_linked" ] && _eflags+=(-e "SOURCE_DIRS=/srv/raw,/srv/linked")
  info "Ingesting raw/${_linked:+ + linked sources} → curated store"
  ...
  dc "${profiles[@]}" run --rm "${_eflags[@]}" ${_vflags[@]+"${_vflags[@]}"} ingest "$@"
  if is_running docs-mcp; then
...
# uninstall [--images] [--force]  — remove the deployment AND its data: containers, the
# index + vector store (docstore + qdrant volumes), tokens.json, .env, groups.json, var/.
# KEEPS your raw/ source docs, the git repo, and vendored models/ (so a reinstall can
# re-ingest). --images also drops the built images (~3.4 GB). Destructive: requires typing
# 'wipe' unless --force (which is also how to run it non-interactively).
cmd_uninstall() {
  need_docker
  local rm_images="" force=""
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --images)         rm_images=1; shift ;;
      --force|--yes|-y) force=1; shift ;;
      *)                die "usage: ./docmcp.sh uninstall [--images] [--force]" ;;
    esac
  done
  warn "uninstall REMOVES: containers, the index + vector store (docstore + qdrant volumes), tokens.json, .env, groups.json, var/.${rm_images:+ Built images are removed too.}"
  warn "KEEPS: your raw/ source docs, the git repo, and vendored models/ (reinstall re-ingests)."
  if [ -z "$force" ]; then
    [ -t 0 ] || die "non-interactive shell — re-run with --force to confirm the wipe"
    printf 'Type %swipe%s to confirm: ' "$C_B" "$C_0"
    local reply; IFS= read -r reply </dev/tty || reply=""
    [ "$reply" = wipe ] || die "aborted — you did not type 'wipe'"
  fi
  info "Stopping services + removing data volumes…"
  dc --profile ingest --profile vector --profile portal down -v || true
  docker rm -f docs-mcp-console >/dev/null 2>&1 || true
  rm -f "$ROOT/tokens.json" "$ROOT/.env" "$ROOT/groups.json" 2>/dev/null || true
  rm -f "$ROOT"/.env.bak.* 2>/dev/null || true
  rm -rf "$ROOT/var" 2>/dev/null || true
  if [ -n "$rm_images" ]; then
    info "Removing built images…"
    docker rmi "$SERVER_IMAGE" "$SERVER_VECTOR_IMAGE" "$INGEST_IMAGE" docs-mcp:console >/dev/null 2>&1 || true
  fi
  info "Uninstalled. Your raw/ docs + repo are intact — reinstall any time: ./docmcp.sh"
}
...
${C_B}docmcp.sh${C_0} — Documentation MCP Server helper (Docker-based; only Docker is required)
  ...
  ${C_B}uninstall${C_0} [--images]        remove the deployment + data (keeps raw/ docs + repo; --images frees ~3.4 GB)
...
case "$cmd" in
  ...
  env-set)            cmd_env_set "$@" ;;
  stop|down)          cmd_stop "$@" ;;
  uninstall|teardown) cmd_uninstall "$@" ;;
  logs)               cmd_logs "$@" ;;
  build)              cmd_build "$@" ;;
  models)             cmd_models "$@" ;;
