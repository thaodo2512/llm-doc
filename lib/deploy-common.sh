...
dep_load_env() {
  ...
  unset HTTP_BIND HTTP_PORT HTTPS_PORT ALLOW_PLAINTEXT_HTTP DOMAIN ALLOWED_HOSTS \
        PORTAL_ENABLED ALLOW_PLAINTEXT_PORTAL ENABLE_VECTOR EMBED_BACKEND OPENAI_API_KEY
...
# _profile_vector VKEY — set the vector-search state idempotently (a declined re-run fully
# clears it, so it never keeps calling a stale backend). Three modes via the VKEY slot:
#   ""         -> OFF              (clear ENABLE_VECTOR + OPENAI_API_KEY)
#   __local__  -> ON, OFFLINE      (local ONNX embedder — no API key, no network)
#   <api-key>  -> ON, OpenAI       (legacy online backend)
_profile_vector() {
  case "$1" in
    "")        env_unset ENABLE_VECTOR; env_unset EMBED_BACKEND; env_unset OPENAI_API_KEY ;;
    __local__) env_set ENABLE_VECTOR true; env_set EMBED_BACKEND local; env_unset OPENAI_API_KEY ;;
    *)         env_set ENABLE_VECTOR true; env_set EMBED_BACKEND openai; env_set OPENAI_API_KEY "$1" ;;
  esac
}
