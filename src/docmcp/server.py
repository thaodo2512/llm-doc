...
def build_asgi_app(settings: Settings):
    ...
    if settings.allowed_hosts:
        ...
        # Fail-open is deliberate (empty ALLOWED_HOSTS is a documented "trusted reverse
        # proxy handles Host" escape hatch) but must be visible — never silent.
        print(
            "[server] WARNING: ALLOWED_HOSTS is empty — Host-header / DNS-rebinding "
            "protection is DISABLED. Set ALLOWED_HOSTS to the host(s)/IP clients use, "
            "unless a trusted reverse proxy already validates the Host header.",
            file=sys.stderr,
        )
    middleware.append(
