...
SH = (Path(__file__).resolve().parents[2] / "docmcp.sh").read_text(encoding="utf-8")
...
def test_link_registers_external_sources_without_copying():
    # `link` creates a visible symlink registry in raw/; `ingest` bind-mounts each link's
    # target into the container and reads it in place (no copy), under /srv/linked.
    link = _body("cmd_link")
    assert "ln -sfn" in link  # creates/updates the symlink
    assert "--remove" in link and "--list" in link  # manage links
    ingest = _body("cmd_ingest")
    assert "/srv/linked/" in ingest  # bind-mounts the link targets read-only
    assert "SOURCE_DIRS=/srv/raw,/srv/linked" in ingest  # adds the linked root for ingest
    assert ":ro" in ingest  # linked sources are mounted read-only
    assert "\n  link)" in SH  # dispatch entry


def test_uninstall_wipes_data_but_keeps_sources_and_is_guarded():
    # The teardown verb removes the deployment + data volumes + secrets/config, but must
    # NOT delete the user's raw/ source docs or the repo — and must be confirmation-gated.
    body = _body("cmd_uninstall")
    assert "down -v" in body  # removes the data volumes (index + qdrant), not just containers
    assert 'rm -f "$ROOT/tokens.json"' in body  # clears minted secrets/config
    assert "wipe" in body and "--force" in body  # typed confirmation, with a non-interactive escape
    assert 'rm -rf "$ROOT/raw' not in body  # NEVER deletes the user's source corpus
    assert "uninstall|teardown)" in SH  # dispatch entry exists


def test_deploy_supports_offline_local_vector():
    # The wizard/deploy must be able to enable vector search with the OFFLINE local model
    # (no OpenAI key) — otherwise an air-gapped operator can't turn on semantic search.
    root = Path(__file__).resolve().parents[2]
    local_deploy = (root / "local_deploy.sh").read_text(encoding="utf-8")
    deploy_common = (root / "lib" / "deploy-common.sh").read_text(encoding="utf-8")
    assert "--vector-local" in local_deploy  # the offline opt-in flag
    assert 'VKEY="__local__"' in local_deploy  # mapped to the local sentinel
    assert "env_set ENABLE_VECTOR true; env_set EMBED_BACKEND local" in deploy_common
    assert "__local__)" in deploy_common  # _profile_vector handles the local sentinel
